...
const StringPopUpHeader = (props) => {
    ...
    const translateService=ewtPageTranslationGlobal.providers;
    const serviceProvideLength=Object.keys(translateService).length;

    return (
        <div className="modal-header" key={props.modalRender}>
            <span className="close" onClick={closeModal}>&times;</span>
            <h2 className="notranslate">{sprintf(__("%sStart Automatic Translation Process", 'easy-wp-translator'), serviceProvideLength > 1 ? 'Step 2 - ' : '')}</h2>
