    ...
    return (
        <div className='mx-auto p-10 max-w-[600px] min-h-[40vh] bg-white shadow-sm flex flex-col'>
            <div className='flex-grow'>
                <h2>{__('Media', 'easy-wp-translator')}</h2>
                <p className='text-justify text-sm/6'>{__('EasyWPTranslator lets you translate media details like the title, alt text, caption, and description. The file isn’t duplicated, but you’ll see a separate entry for each language in the media library. When adding media to a post, you’ll only see options in that post’s language.', 'easy-wp-translator')}</p>
                <p className='text-justify text-sm/6'>{__('Turn on media translation if you need to translate the title, alt text, caption, or description. If not, you can leave it off.', 'easy-wp-translator')}</p>

                <div className='flex justify-between items-center p-6 rounded-lg' style={{ border: "1px solid #e5e7eb" }}>
                    <p className="text-sm/6">{__('Allow EasyWPTranslator to translate media', 'easy-wp-translator')}</p>
                    <Switch
